...
| - | -------- | ----------------- |
| RF-21.1 | La franja va **dentro de la barra del sitio**, después de la fila de navegación, y **no lleva ningún texto encima**. Al cargar, la barra mide 237 px con la fotografía; cerrada la franja queda en 71 px | `SiteHeader.astro`; `[medido: 2026-08-09]`; `npm run verify` = 0 indeterminados |
...
| RF-21.2b | La altura es `clamp(9rem, 21vw, 16rem) × 0,65`, por indicación de Daniel el 2026-08-09. La multiplicación se deja a la vista en lugar de tres números ya resueltos, para que se siga viendo de dónde salió el `clamp` | `--alto-franja` en `global.css` |
| RF-21.3 | Dos capas sobre la imagen: velo plano al 18 % y **degradado en el borde superior**, a todo lo ancho. El degradado va arriba y no abajo: encima está la fila de navegación, y `superficie-institucional` redefine `--background` al azul PUCV en este subárbol, así que la fotografía **nace de la barra**. Abajo el canto es duro, contra el hero, y marca dónde acaba la barra | inspección |
| RF-21.4 | El velo **no depende del tema**, y es consecuencia de estar dentro de la barra: el azul institucional es el mismo en los dos temas. Cuando la franja estaba en `main` el velo tomaba el fondo de la página y hubo que corregirlo de 18 % a 48 % para el tema oscuro; aquí esa corrección sobra. En oscuro sí baja la saturación al 0,88: el cielo competía con el único acento cromático del tema | inspección |
| RF-21.5 | La franja se **cierra encadenada al arrastre**, no a un umbral: empieza en el primer píxel de desplazamiento y su altura es función de cuánto se arrastró, así que se puede parar a medio camino y volver atrás. Medido a 1280×820: 166 → 136 → 106 → 76 → 46 → 0 px a 0, 30, 60, 90, 120 y 166 px de scroll, y la barra aterriza en 71 px exactos | `[medido: 2026-08-09]`; `animation-timeline: scroll(root block)` |
| RF-21.5b | Se implementa con **longhands, nunca con el atajo `animation`**. El atajo restablece `animation-duration` a `0s` al omitirla y con duración cero la línea de tiempo no se aplica: la franja se quedaba a 166 px a cualquier desplazamiento. Hace falta `animation-duration: auto` | `[medido: 2026-08-09]` |
| RF-21.5c | Respaldo por umbral —cierre de una vez al dejar atrás el titular— para navegadores sin `animation-timeline` (hoy Firefox y Safari) **y** para quien pide movimiento reducido. No necesita `@supports not`: donde la animación corre, gana en la cascada y acaba en 0, que es lo mismo que dice la regla de respaldo | umbral `data-hero-visible` |
| RF-21.5d | La franja se recoge mientras el menú móvil está abierto: es un `<details>` en flujo y empujaría la fotografía hasta media pantalla. Con `!important`, porque compite con la animación | `header:has([data-mobile-menu][open])` |
